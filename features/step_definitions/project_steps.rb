...
Then /^the form for adding a note should not be visible$/ do
  expect(page).to_not have_css("edit_form_note_section")
end
...
Then /^the project title should be "(.*)"$/ do |title|
  expect(page).to have_css("h2#project_name_container span", text: title, exact: true)
end
...
Then /^I should (see|not see) the default project settings$/ do |visible|
  default_setings = "This project is active with no default context and with no default tags"
